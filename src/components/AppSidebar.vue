<script src="../../../../onlineDemo/offline-wikipedia-master/public/js/sw/index.js"></script>
<template>
                        ...
                        <ul v-if="block.list">
                            <li v-for="item in block.list" @click.stop="closeAndGo(item.route)">
                                <span v-if="item.icon || item.image || item.svg " class="app-sidebar-block-left-icon">
                                    <img v-if="item.image" :src="item.route === topPath ? item.image.replace(/\.png/,'-active.png') : item.image" :alt="item.alt" />
